...
from util_SEresnet import train_model
...
SEresnet = ResSENet18().to(device)
#count number of params
def count_parameters(model):
    return sum(p.numel() for p in model.parameters() if p.requires_grad)

print(f'number of trainable parameters: {count_parameters(SEresnet)}')

# Optimizer and loss function
criterion = nn.CrossEntropyLoss().to(device)
params_to_update = SEresnet.parameters()
optimizer = optim.Adam(params_to_update, lr=0.01)
#%% 
#resnet.is_debug = True
best_model, val_acc_history, loss_acc_history = train_model(SEresnet, dataloaders, criterion, optimizer, 25, 'SEresnet18_bestsofar')
...
torch.save(best_model.state_dict(), 'se-resnet-18-cifar-10.pth')

torch.save(val_acc_history, 'se-resnet-18-cifar-10-val-acc.pth')
torch.save(loss_acc_history, 'se-resnet-18-cifar-10-loss-acc.pth')