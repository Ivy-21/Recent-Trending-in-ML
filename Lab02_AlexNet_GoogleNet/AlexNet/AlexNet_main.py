...
#%%
import Alexnet_sq
import Alexnet_sq_LRN
from Alexnet_module import AlexNetModule
from Alexnet_module_LRN import AlexNetModuleLRN

sequential = Alexnet_sq.alexnet_sequential
sequential_LRN = Alexnet_sq_LRN.alexnet_sequential_LRN
module = AlexNetModule(10)
module_LRN = AlexNetModuleLRN(10)

models = [sequential,sequential_LRN,module,module_LRN]
...
criteriion = criteriion.to(device)
...
for i, model in enumerate(models):
    ...
    best_model, val_acc_history, loss_acc_history = train_model(model, dataloaders, criteriion, optimizers[i], 10, model_names[i])
...
for i, model in enumerate(models):
    ...
    #test_loss, test_acc, test_pred_label, test_true_label = evaluate(model, test_dataloader, criteriion,model_names[i])
    test_loss, test_acc, test_pred_label, test_true_label  = evaluate(model, test_dataloader, criteriion)
